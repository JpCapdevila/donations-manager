<script>
  ...
  onMount(async () => {
    campaigns = await SerializedDocumentArray.fromQuery(query(collection(getFirestore(), "campaigns")));
  });
</script>
...
<div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
            <h1 class="text-xl font-semibold text-gray-900">Campañas</h1>
        </div>
                ...
                <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-300">
                        ...
                        {#each campaigns as campaign (campaign.ref.id)}
                            <tr>
                                <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{campaign.data.name}</td>
                                <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                    <a href={buildRelativePath($page,`./${campaign.ref.id}`)}
                                       class="text-rose-600 hover:text-rose-900 mr-4">Editar</a>
                                    <a href={buildRelativePath($page,`./${campaign.ref.id}/donations`)}
                                       class="text-rose-600 hover:text-rose-900">Registrar Donativo</a>
                                </td>
                            </tr>
                        {/each}
                        </tbody>
